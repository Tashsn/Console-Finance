...
let result = finances.reduce((a, b, i) => {
  if (i === 0) {
    ...
    let change = b[1] - a.lastMonth;
    a.changes.push(change); 
    a.total += b[1];
    a.sumChange += change; 
    a.lastMonth = b[1];
    if (change > a.increase[1]) { 
      a.increase = [b[0], change];
    }
    if (change < a.decrease[1]) { 
      a.decrease = [b[0], change];
    }
    return a;
  }
}, {});

result.average = result.sumChange / (length - 1); 
